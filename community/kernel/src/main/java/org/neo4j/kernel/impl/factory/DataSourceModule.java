...
/**
 * Datasource module for {@link org.neo4j.kernel.impl.factory.GraphDatabaseFacadeFactory}. This implements all the remaining
 * services not yet created by either the {@link org.neo4j.kernel.impl.factory.PlatformModule} or {@link org.neo4j.kernel.impl.factory.EditionModule}.
 *
 * When creating new services, this would be the default place to put them, unless they need to go into the other modules for any reason.
 */
public class DataSourceModule
{
    ...
    public DataSourceModule(final GraphDatabaseFacadeFactory.Dependencies dependencies, final PlatformModule platformModule, EditionModule editionModule)
    {
        ...
        StoreFactory storeFactory = new StoreFactory( config, editionModule.idGeneratorFactory, platformModule.pageCache, fileSystem,
                        logging.getInternalLogProvider(), platformModule.monitors );
        ...
        Guard guard = config.get( GraphDatabaseFacadeFactory.Configuration.execution_guard_enabled ) ? deps.satisfyDependency( new Guard( logging.getInternalLog( Guard.class )) ) : null;
